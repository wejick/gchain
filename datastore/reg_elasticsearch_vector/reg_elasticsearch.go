package reg_elasticsearch_vector
...
type ElasticsearchVectorStore struct {
	regularEsClient *elasticsearch.Client
...
func NewElasticsearchVectorStore(host string, embeddingModel model.EmbeddingModel, esOption ...func(*ESOption)) (EVS *ElasticsearchVectorStore, err error) {
	...
	regularEsClient, err := elasticsearch.NewClient(cfg)
	if err != nil {
		return
	}
	EVS = &ElasticsearchVectorStore{
		regularEsClient: regularEsClient,
...
func (ES *ElasticsearchVectorStore) AddDocuments(ctx context.Context, indexName string, documents []document.Document) (batchErr []error, err error) {
	...
	for _, esDoc := range esDocs {
		...
		response, err := ES.regularEsClient.Index(
			indexName,
			strings.NewReader(jsonDoc),
			ES.regularEsClient.Index.WithContext(ctx),
		)
...
func (ES *ElasticsearchVectorStore) DeleteIndex(ctx context.Context, indexName string) (err error) {
	_, err = ES.regularEsClient.Indices.Delete(
		[]string{indexName},
		ES.regularEsClient.Indices.Delete.WithContext(ctx),
	)
...
func (ES *ElasticsearchVectorStore) SearchVector(ctx context.Context, indexName string, vector []float32, options ...func(*datastore.Option)) (docs []document.Document, err error) {
	...
	// log.Println("============ jsonVector =================")
	// log.Printf("%+v\n", jsonVector)

	resp, err := ES.regularEsClient.API.KnnSearch(
		[]string{indexName},
		ES.regularEsClient.API.KnnSearch.WithContext(ctx),
		ES.regularEsClient.API.KnnSearch.WithBody(strings.NewReader(jsonVector)),
	)
...
func (ES *ElasticsearchVectorStore) createIndexIfNotExist(ctx context.Context, indexName string) (err error) {
	...
	resp, err := ES.regularEsClient.Indices.Create(
		indexName,
		ES.regularEsClient.Indices.Create.WithBody(strings.NewReader(default_mapping)),
		ES.regularEsClient.Indices.Create.WithContext(ctx),
	)
	if err != nil {
...
func (ES *ElasticsearchVectorStore) isIndexExist(ctx context.Context, indexName string) (exist bool, err error) {
	resp, err := ES.regularEsClient.API.Indices.Exists(
		[]string{indexName},
		ES.regularEsClient.API.Indices.Exists.WithContext(ctx),
	)
	if err != nil {
		return false, err
	}
